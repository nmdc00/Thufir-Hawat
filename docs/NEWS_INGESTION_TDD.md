...
At 816 posts/day, the binary replay mean implies roughly 35 minutes/day of first-pass local inference before urgency checks, contention, and retries. The local model is resident and local calls do not count against the configured hourly LLM budget. The risk is scheduling: `createTrivialTaskClient()` holds the shared global LLM queue while using the separate single-flight local queue, and both queues are FIFO. A long news backlog can therefore delay chat, originator, and other LLM work even when host CPU and memory are available. The design must solve this queue interaction before full-feed screening is active.
...
15. A burst of news jobs does not hold the global LLM queue while waiting for Ollama, and a new interactive/originator request can overtake waiting news jobs.
