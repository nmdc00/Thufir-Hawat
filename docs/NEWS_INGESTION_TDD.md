...
- **Production model prompt correction:** A labeled post-deployment replay exposed a gold false negative with the original relevance wording. A prompt that explicitly treats current price moves across broad financial-market classes as relevant produced the expected `YES` for gold, Brent, and diesel and `NO` for the conditional Iran warning, unrelated pharmaceutical story, and generic digest in three consecutive six-headline runs on the deployed local `qwen2.5:1.5b-instruct` model. This small replay checks these examples; it is not a general accuracy guarantee.
