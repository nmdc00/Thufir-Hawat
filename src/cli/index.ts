...
function buildEnvContent(values: Record<string, string>): string {
  const lines = ['# Bijaz environment variables', '# Generated by bijaz env init', ''];
...
async function runEnvChecks(values: Record<string, string>): Promise<void> {
  ...
  if (env.BIJAZ_KEYSTORE_PATH) {
    const exists = existsSync(env.BIJAZ_KEYSTORE_PATH);
    results.push({
      name: 'Keystore',
      ok: exists,
      detail: exists ? 'found' : 'missing BIJAZ_KEYSTORE_PATH file',
    });
  } else {
    results.push({ name: 'Keystore', ok: false, detail: 'missing BIJAZ_KEYSTORE_PATH' });
  }
...
markets
  ...
  .action(async (id) => {
    ...
    try {
      ...
      console.log('You can find this in the market URL or by using `bijaz markets search`.');
    }
  });
