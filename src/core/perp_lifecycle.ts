...
export function buildPerpExecutionLearningCase(
  ...
  const thesisScore = input.thesisCorrect == null ? null : input.thesisCorrect ? 1 : 0;
  ...
  const thesisPathScore = meanOrNull([
    ...
    toFiniteOrNull(input.capturedR) != null
      ? Math.max(0, Math.min(1, Number(input.capturedR) / 2))
      : null,
    toFiniteOrNull(input.mfeProxy) != null
      ? Math.max(0, Math.min(1, Number(input.mfeProxy)))
      : null,
  ]);
  const thesisCompositeScore = meanOrNull([
    thesisScore,
  ...
  return {
    ...
    pairedCases: [
      buildPerpThesisLearningCaseInput({
        ...
        sourceLinks: { snapshot: input.snapshot },
      }),
    ],
  };
}
