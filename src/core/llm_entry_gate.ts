...
function formatTrackRecord(stats: SignalPerformanceSummary, allowPaperColdStart: boolean): string {
  if (stats.sampleCount === 0) {
    if (allowPaperColdStart) {
      return `No historical trades for signal class "${stats.signalClass}" yet. This is a bounded paper-mode exploration candidate. Do not reject solely because sample count is zero; require a concrete stop, supported target/R:R, TTL, and acceptable liquidity/execution evidence.`;
    }
...
${formatTrackRecord(signalStats, allowPaperColdStart)}
