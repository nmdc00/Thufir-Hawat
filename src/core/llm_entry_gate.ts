...
import { summarizeComparableSignalPerformance, type SignalPerformanceSummary } from './signal_performance.js';
...
export interface EntryGateCandidate {
  ...
  mechanicalMinEdge?: number | null;
  mechanicalMinConfidence?: number | null;
  ...
  if (stats.sampleCount === 0) {
    ...
    const observedNoise =
      stats.observedCount > 0
        ? ` Recent activity exists (${stats.blockedCount} blocked, ${stats.unresolvedCount} unresolved), but there are no resolved comparable trades yet.`
        : '';
    return `No resolved comparable trade history for signal class "${stats.signalClass}". Treat as a novel setup — apply extra scrutiny.${observedNoise}`;
  }
  const winPct = (stats.thesisCorrectRate * 100).toFixed(0);
  const credibility = stats.sampleCount < 5 ? ' (low sample — high uncertainty)' : '';
  const scopeLabel =
    stats.scopeLevel === 'signal_class_and_symbol_class_and_regime'
      ? 'signal + symbol class + regime'
      : stats.scopeLevel === 'signal_class_and_symbol_class'
        ? 'signal + symbol class'
        : 'signal class';
  const excludedNoise =
    stats.blockedCount > 0 || stats.unresolvedCount > 0
      ? ` Excluded ${stats.blockedCount} blocked and ${stats.unresolvedCount} unresolved attempts from realized trade stats.`
      : '';
  return [
    `Signal class: ${stats.signalClass} — ${stats.sampleCount} resolved trades from ${scopeLabel}${credibility}`,
    `Win rate: ${winPct}% | Expectancy: ${stats.expectancy.toFixed(2)} | Sharpe-like: ${stats.sharpeLike.toFixed(2)}`,
    `Avg adverse move: ${stats.maeProxy.toFixed(3)} | Avg favorable move: ${stats.mfeProxy.toFixed(3)}${excludedNoise}`,
  ].join('\n');
}
...
function buildPrompt(
  ...
  const configuredEdgeFloorPct = ((candidate.mechanicalMinEdge ?? 0) * 100).toFixed(2);
  const configuredConfidenceFloorPct = ((candidate.mechanicalMinConfidence ?? 0) * 100).toFixed(1);
...
- suggestedLeverage: only set when verdict is "approve" or "resize". Pick an integer from 1 to leverageMax. Use 1x by default. Treat edge >10% and confidence >70% as a leverage scale-up bar, not as a general approval threshold. Do not reject solely because edge is below 10% if the setup already clears the configured mechanical floor and the thesis is otherwise sound. Scale up only when ALL of the following hold: edge >10%, confidence >70%, clear directional regime (trending or expansion), deep liquidity, and a well-defined stop. Use maximum leverage only for exceptional setups. Omit (or set to 1) if you have any doubt.
...
- Mechanical floor already passed: edge >= ${configuredEdgeFloorPct}% and confidence >= ${configuredConfidenceFloorPct}%
- Signal class: ${candidate.signalClass}
- Symbol class: ${candidate.symbolClass ?? 'unknown'}
...
export class LlmEntryGate {
  ...
  async evaluate(
    ...
    const signalStats = summarizeComparableSignalPerformance(
      listPerpTradeJournals({ limit: 200 }),
      {
        signalClass: candidate.signalClass,
        symbolClass: candidate.symbolClass ?? null,
